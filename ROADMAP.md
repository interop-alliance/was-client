...
## A transport-free entry for the offline codec

Filed 2026-09-18 from a scoping pass run for `@interop/wallet-backup` (its WBU-3
is the consuming item). That package opens archived bytes offline. It derives
keys, reads an encryption descriptor out of an archived resource log, and
decrypts rows through `createEdvDocCipher`. It never talks to a server.
Importing `@interop/was-client/edv` still evaluates the whole encrypted entry:
`WasTransport`, `createEdvEncryption`, the descriptor refresh policy, the
request wrapper in `internal/request.ts`, and through the `@interop/edv-client`
barrel the HTTP client and the zcap HTTP signature package. None of that code
runs for an offline caller. The cost is module evaluation and bundle size. It is
not a correctness problem, and install size does not change, since was-client
keeps every dependency it has today.

This is restructuring within the package. A separate package for `src/edv/` was
considered again in this pass and set aside. ARCHITECTURE.md's "Subpaths, not
packages" decision (2026-08-22) names its revisit criteria, and an offline
consumer meets none of them: it wants the crypto packages installed and only
wants the transport modules out of its import graph. A subpath entry gives
exactly that. No export is removed by any item here. Everything
`@interop/was-client/edv` exports today keeps resolving from it.

WCL-111 landed 2026-09-18 and is archived: `src/edv/` now imports the EDV client
through its `./core` entry, so the HTTP client and the zcap signature package
are already out of the `@interop/was-client/edv` graph. What the paragraph above
describes as still evaluated is, from that date, `WasTransport`,
`createEdvEncryption`, the descriptor refresh policy and `internal/request.ts`.

The four items are ordered by dependency. WCL-111, WCL-112 and WCL-113 each
remove one reach into transport code and are behavior-preserving on their own.
WCL-114 adds the entry and the test that keeps the graph clean, and it delivers
nothing until the other three land. All four are low priority: nothing is
broken, and the work is worth scheduling when a consumer has a concrete need (a
bundle-size problem under Metro, or a runtime where the HTTP packages cannot
load).

The scoping pass sized the `EdvCodec.ts` cut as a split of the 1400-line class.
Reading the file shows a smaller cut. The class already takes its transport
through an injected `CodecTransportFactory`, so WCL-113 moves two functions and
...
- acceptance:
  - [ ] `src/internal/describe.ts` has no import from `./request.js`, runtime or
        type
  - [ ] `readCollectionMetadata` lives in a module that is allowed to import the
        request wrapper, and `Collection.ts` and `Space.ts` import it from there
  - [ ] `src/edv/descriptorStore.ts` and `src/edv/logGovernedDescriptorStore.ts`
        no longer reach `internal/request.ts` through their static imports
  - [ ] No behavior change: the existing node and browser suites pass unedited
...
- touches:
  - was-client (this repo): ARCHITECTURE.md's `EdvCodec` and `WasTransport`
    paragraphs (around lines 321-345), which describe where the transport is
    built
- acceptance:
  - [ ] `src/edv/EdvCodec.ts` imports `WasTransport` as a type only
  - [ ] `wasTransportFactory` and `createEdvEncryption` live outside
        `EdvCodec.ts`, in a module (or modules) on the online side, and
        `edv/index.ts` still exports both under the same names
  - [ ] `createEdvDocCipher` called without `spaceId` evaluates no module that
        imports `WasTransport`. Called with `spaceId`, it behaves as today,
        including the chunked-stream paths
  - [ ] `createEdvEncryptOnlyDocCipher`, `buildEdvCodec` and
...
- touches:
  - was-client (this repo): package.json `exports`; ARCHITECTURE.md's layering
    section, the "Subpaths, not packages" section (the new entry is a further
    use of that decision, and the section should say so), a new numbered
    invariant beside invariant 9, and a Glossary entry for the entry's name;
    README.md's encrypted collections section, which introduces the `./edv`
    subpath
  - wallet-backup: WBU-3 repoints `createEdvDocCipher`,
    `createEdvEncryptOnlyDocCipher`, `EPOCH_CONFIGURATION_STATE_TYPE` and its
    fixture imports to the new entry, and adds its own import-graph test
  - wallet-core: `keyring/record.ts`, `keys/userKey.ts` and
    `keys/userKeyCascade.ts` import cipher, epoch and recipient functions from
    `@interop/was-client/edv`. Its own transport-free subpaths depend on this
    entry. File the follow-up there
  - was-react, freewallet, dcw: online consumers. Expected
    `unaffected: <repo> (imports keep resolving from ./edv)`, to be confirmed
    per repo
  - encrypted-collections-spec: AGENTS.md "Parties to this contract" row for
    was-client names `src/edv/` as the reference implementation. Update the row
    if the entry changes which modules it should cite
- acceptance:
  - [ ] package.json `exports` gains one entry, with the four keys the other
        entries carry. The name is the maintainer's call and is settled before
        any code lands
  - [ ] The entry re-exports the offline set and nothing else: the doc cipher
        ...
        wallet-core import
  - [ ] `@interop/was-client/edv` re-exports the new entry and adds the online
        ...
        list is unchanged
  - [ ] `test/node/import-graph.test.ts` gains a block for the new entry. It
...
that re-exports the online and offline halves side by side (`WasTransport` at
line 92, `createEdvEncryption` through the `EdvCodec.js` block ending at line
90, the refresh policy and refreshing cipher at lines 154-155). ESM evaluates
every module a barrel names, whichever export the importer wants. An offline
consumer needs an entry whose barrel names only offline modules. That is the
same mechanism the package already uses to keep the core entries off the
